...
<body>
	<header class="container mt-5">
		...
		<% } else { %>
			...
			<a style="float:right;" href="./memberSearchForm.do" onclick="window.open(this.href, '_blank', 'width=500px,height=300px,toolbars=no,scrollbars=no'); return false;">멤버 관리</a></b>
			</div>
		<% } %>
	</header>
	...
	<section class = "container" id="pageList">
		...
		[이전]&nbsp;
		<%}else{ %>
		<a href="totalSearch.do?page=<%=nowPage-1 %>&&keyword=<%=keyword%>">[이전]</a>&nbsp;
		<%} %>
		...
		[<%=a %>]
		<%}else{ %>
		<a href="totalSearch.do?page=<%=a %>&&keyword=<%=keyword%>">[<%=a %>]
		</a>&nbsp;
		<%} %>
		<%} %>
		...
		[다음]
		<%}else{ %>
		<a href="totalSearch.do?page=<%=nowPage+1 %>&&keyword=<%=keyword%>">[다음]</a>
		<%} %>
	</section>
